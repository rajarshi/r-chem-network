...
it has become apparent that this model does not always match
reality. Instead, it has become increasingly clear that many drugs
...
activity against hERG led to an increased risk of cardiac death. But
...
diabetes and various cancers. In parallel, even when a single target
is the main driver for a given disease state, compensatory pathways
can ``route around'' inhibition of the primary target, thus restoring
the functional behavior that the primary target is responsible (i.e.,
the system is now resistant to the original drug)
...
then identifying ways to modulate them. The remainder of this chapter
looks at how network methods can be applied to various topics in drug
discovery ranging from drug-target interactions to analyzing
